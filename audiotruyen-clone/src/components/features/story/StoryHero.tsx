                    ...
                    <div className="flex flex-1 flex-col justify-start space-y-4">
                        ...
                        <div className="grid grid-cols-1 gap-y-2 gap-x-8 text-sm sm:grid-cols-2 lg:max-w-2xl">
                            <div className="flex items-center gap-2 text-[var(--color-text-secondary)]">
                                ...
                                <span className="font-semibold text-[var(--color-primary)]">{story.author}</span>
                            </div>
                        ...
                        <div className="flex items-center gap-4 border-t border-[var(--color-border-light)] pt-3">
                            <div className="flex items-center gap-1">
                                <span className="text-2xl font-bold text-[var(--color-text-primary)]">{story.rating}</span>
                                <div className="flex text-amber-400">
                                    {Array.from({ length: 5 }).map((_, i) => (
                                        <svg key={i} className={`h-5 w-5 ${i < Math.floor(story.rating) ? 'fill-current' : 'text-slate-200 fill-current'}`} viewBox="0 0 20 20">
                            ...
                            <span className="text-sm text-[var(--color-text-muted)]">{story.ratingCount} đánh giá</span>
                        </div>
