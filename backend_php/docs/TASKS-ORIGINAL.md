...
- [x] Route: `GET /v1/public/categories/{slug}` (Chi tiết)
- [x] Route: `GET /v1/public/stories/slugs-all` (Lấy tất cả slug)

### 3.5 API Management (Chapters)
- [x] Route: `POST /v1/chapters/save` (Thêm/Sửa chương)
- [x] Route: `DELETE /v1/chapters/{id}` (Xóa chương)
- [x] Logic: Update `total_chapters` in `stories` table after changes
...
- [/] Review toàn bộ code & Fix lỗi nhỏ
  ...
  - [ ] Thêm `deleted_at IS NULL` cho `StoryRepository` (Rejected by User)
  - [x] Refactor `StoryController` dùng `RequestValidator`
  - [x] Bảo mật API Story (Admin Only) cho `save` và `delete`
  - [x] Hỗ trợ Bulk Save (Lưu nhiều truyện cùng lúc) trong API `save`
